...
const DisplayCampaigns = ({ title, notFound, isLoading, setHasDeleted, campaigns = [] }) => {
  ...
  return (
    <div>
      <h1 className="font-epilogue font-semibold font-['Ubuntu'] text-xl md:text-3xl text-white text-center md:text-left">
        ...
        {!isLoading && campaigns.length > 0 && campaigns.map((campaign) => (
          <FundCard
            ...
            editable={
              state.loggedIn &&
              state.loggedUser &&
              state.loggedUser.yourCampaigns &&
              state.loggedUser.yourCampaigns.find(item => item.campaign.campaign_id === campaign._id)
            }
            handleClick={() => {
              if (state.loggedIn) {
